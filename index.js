...
firebase.auth().onAuthStateChanged(async function(user) {
  ...
  // Build the URL for our nft API
   ...
   for (let i=0; i < json.length; i++) {
     ...
     <p class="md:ml-4 ml-2 capitalize font-bold text-xl">${nft.name}</p>

       <div class="md:flex md:ml-4 ml-2 my-2">
         <img src="${nft.urlNft}" class="w-1/3 border-2 border-blue-400 rounded">
        <div>
           <p class="md:ml-4  font-bold text-base capitalize ">Item Description: ${nft.description}</p>
           <p class="md:ml-4  font-bold text-base capitalize ">Category: ${nft.category} </p>
           <p class="md:ml-4  font-bold text-base capitalize">Price: $${nft.price}</p>
